...
Slack, Gmail, Calendar) backed by live API calls to each real service.
...
Per-integration credentials (JIRA_*, GITHUB_*, SLACK_*, GOOGLE_*) are read by
server/config.py — see README.md for how to obtain each one. Write actions
(update_jira_ticket, post_slack_message, send_email) are read-only/disabled
unless WRITES_ENABLED=true; when enabled, they still only draft a change and
return a pending_action_id — nothing is applied until a human approves and
the caller invokes confirm_action(pending_action_id=...), which is the only
place that actually calls the real write API.
"""
...
from . import actions
from .clients import calendar_client, github_client, gmail_client, jira_client, slack_client
from .clients.errors import guarded
...
INSTRUCTIONS = (
    ...
    "by live calls to the real services. Call get_user_profile() to see whose data "
    "this is; 'today'/'yesterday'/'this_week' in filters resolve against the real "
    "wall-clock date. Every factual claim must trace to a tool result — never "
    "invent ticket IDs, PR numbers, meeting times, or Slack messages; if something "
    "isn't found, say so or call escalate_to_user. update_jira_ticket, "
    "post_slack_message, and send_email are disabled unless the server operator "
    "has set WRITES_ENABLED=true; when enabled they only DRAFT a change and "
    "return a pending_action_id — nothing is applied until a human approves and "
    "you call confirm_action(pending_action_id=...). Never tell the user an "
...
def _writes_disabled(action: str) -> dict:
    return {
        "status": "disabled",
        "action": action,
        "message": (
            "Write actions are currently disabled (WRITES_ENABLED is not set to "
            "true on the server). This is a read-only deployment for now."
        ),
    }
...
def get_jira_tickets(
    ...
    return jira_client.query_tickets(
...
def update_jira_ticket(ticket_id: str, fields: dict) -> dict:
    ...
    if not settings.writes_enabled:
        return _writes_disabled("update_jira_ticket")
    ticket = jira_client.get_issue(ticket_id)
    if ticket is None:
        return _not_found("Jira ticket", ticket_id)
    record = actions.draft_action("update_jira_ticket", {"ticket_id": ticket_id, "fields": fields})
    return {
...
def get_github_prs(
    ...
    return github_client.query_prs(
...
def get_github_pr_detail(pr_id: str) -> dict:
    ...
    pr = github_client.get_pr_detail(pr_id)
    if pr is None:
...
def get_github_commits(
    ...
    return github_client.query_commits(repo=repo, author=author, since=since)


@mcp.tool()
@guarded("GitHub")
def link_jira_to_github(ticket_id: str, pr_url: str | None = None) -> dict:
    """Find the GitHub PR(s) linked to a Jira ticket, by scanning commit
    messages for the 'PROJ-101: ...' convention and resolving matching
    commits to their PR(s). If pr_url is given, also checks whether that
    specific PR is one of the linked ones."""
    ticket = jira_client.get_issue(ticket_id)
    if ticket is None:
        return _not_found("Jira ticket", ticket_id)
    prs = github_client.prs_for_ticket(ticket_id)
    linked = [{**p, "url": github_client.pr_web_url(p)} for p in prs]
    result = {
        "ticket_id": ticket_id,
        "linked_prs": linked,
    ...
    if pr_url is not None:
        target = github_client.find_pr_by_url(pr_url)
...
def post_slack_message(channel: str, message: str) -> dict:
    ...
    and you call confirm_action(pending_action_id=...)."""
    if not settings.writes_enabled:
        return _writes_disabled("post_slack_message")
    record = actions.draft_action("post_slack_message", {"channel": channel, "message": message})
    return {
        "status": "pending_approval",
        "pending_action_id": record["id"],
...
def get_calendar_events(
    ...
    optionally filtered by event_type or participant username. event_type is a
    best-effort match (real Calendar events have no such field natively)."""
    return calendar_client.query_events(
...
def get_gmail_threads(
    ...
    return gmail_client.query_threads(
...
def send_email(to: list[str], subject: str, body: str) -> dict:
    ...
    if not settings.writes_enabled:
        return _writes_disabled("send_email")
    sender = settings.user_profile.email
    record = actions.draft_action("send_email", {"to": to, "subject": subject, "body": body, "sender": sender})
    return {
        "status": "pending_approval",
        "pending_action_id": record["id"],
        "action": "send_email",
...
def confirm_action(pending_action_id: str) -> dict:
    ...
    return actions.confirm_action(pending_action_id)


@mcp.tool()
def get_user_profile() -> dict:
    """Get the current developer's profile (name, team, role, working hours, timezone).
    This is local server config, not fetched from any single integration — no one
    API returns "team/role/working hours" the way a unified profile would."""
...
def escalate_to_user(reason: str) -> dict:
    ...
    actions.sent_log.append({"type": "escalation", "reason": reason})
